...
public class GridSystem : Singleton<GridSystem> {
    ...
    // // ช่องที่ไม่ได้ใช้งาน (null = ตัดออก)
    ...
    // // [row, col] → SlotView หรือ null ถ้าช่องนั้นตัดออก
    // private SlotView[,] _grid = new SlotView[GridSize, GridSize];
    ...
    //             if (type.HasValue) {
    //                 _grid[row, col] = new SlotView(type.Value, new Vector2Int(row, col));
    //             }
    //         }
    //     }
    // }
    ...
    // /// </summary>
    // public SlotView GetSlot(int row, int col) {
    //     if (row < 0 || row >= GridSize || col < 0 || col >= GridSize) return null;
    //     return _grid[row, col];
    // }

    // public SlotView GetSlot(Vector2Int position) => GetSlot(position.x, position.y);
    ...
    //     SlotView slotA = GetSlot(origin);
    //     SlotView slotB = GetSlot(secondSlot);
    ...
    //             SlotView neighbor = GetSlot(pos + dir);
